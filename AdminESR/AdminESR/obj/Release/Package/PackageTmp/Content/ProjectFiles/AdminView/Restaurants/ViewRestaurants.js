function RestaurantDetailsOpenFunction(id) {
    ...
    $.ajax({
        type: "GET",
        url: "http://admin.eatsleeprepeatesr.com/api/restaurants/GetAllRestaurants?id=" + id + "", //URI
        dataType: "json",
        success: function (data) {
            ...
            $.each(myJsonObject, function (i, mobj) {
                ...
                $("#RestaurantDetailsDataBody").append('<tr id="Row' + mobj.Id + '"> ' +
                    ...
                    '               <td>' + mobj.Area + ' </td>' +
                    '               <td>' + mobj.Discount + '% </td>' +
                    '               <td class="mailbox-name"><a href="#/" data-toggle="modal" data-target="#squarespaceModal1" id="seeTags" onclick="seeTags(' + mobj.Id + ')" >See All</a></td>' +
                    '               <td class="mailbox-name"><a href="#/" data-toggle="modal" data-target="#squarespaceModal2" id="seeDeals" onclick="seeDeals(' + mobj.Id + ')" >See All</a></td>' +
                    '               <td>' + mobj.AddDate + ' </td>' +
...
function seeDeals(id) {
    $("#RestaurantsDeals").empty();
    $.ajax({
        type: "GET",
        url: "http://admin.eatsleeprepeatesr.com/api/restaurants/GetAllRestDealsById?id=" + id + "", //URI
        dataType: "json",
        success: function (data) {
            debugger;
            var datavalue = data;
            ...
            contentType: "application/json";
            $.each(myJsonObject, function (i, mobj) {
                $("#RestaurantsDeals").append('<tr id="jawad' + mobj.Id + '"> ' +
                    '<td>' + mobj.Deals + ' </td>' +
                    '</tr>');
            });
        },
...
function seeTags(id) {
    $("#RestaurantsTags").empty();
    $.ajax({
        type: "GET",
        url: "http://admin.eatsleeprepeatesr.com/api/restaurants/GetAllRestTagsById?id=" + id + "", //URI
        dataType: "json",
        success: function (data) {
            debugger;
            var datavalue = data;
            ...
            contentType: "application/json";
            $.each(myJsonObject, function (i, mobj) {
                $("#RestaurantsTags").append('<tr id="jawad' + mobj.Id + '"> ' +
                    '<td>' + mobj.Tags + ' </td>' +
                    '</tr>');
            });
        },
            ...
            callback: function (result) {
                if (result) {
                    $("input:checkbox[name=checkboxDelete]:checked").each(function () {
                        ...
                        $.ajax({
                            type: 'DELETE',
                            url: "http://admin.eatsleeprepeatesr.com/api/restaurants/DeleteRestaurant?id=" + $(this).val() + "", //URI
                            dataType: "json",
                            contentType: "application/json",
...
function GetSingleRestaurantByName() {
    ...
    $.ajax({
        type: "GET",
        url: "http://admin.eatsleeprepeatesr.com/api/restaurants/GetAllRestaurantsByName?id=" + document.getElementById("RestaurantSearch").value + "", //URI
        dataType: "json",
        success: function (data) {
            debugger;
            var datavalue = data;
